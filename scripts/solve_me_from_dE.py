...
def main():
    ...
    # Get the energy difference for the unperturbed states
    delta_E_0 = np.array([fit[f"order3_fit"][:, 1] for fit in fitlists[3]][0])
    print(f"delta_E_0 = {np.average(delta_E_0)}")

    solve_me(lambdas3, fitlists, datadir, fitfunc3, delta_E_0)
...
def solve_me(lambdas3, fitlists, datadir, fitfunction, delta_E_fix):
    """
    Take the energy shifts for a range of lambda values, as well as the unperturbed energy gap.
    Solve for the matrix element by inverting the fit equation at each lambda value.
    Plot these matrix element values for the highest two orders in the expansion as bands.
    """

    order = 3
    DeltaE = np.array([fit[f"order{order}_fit"][:, 1] for fit in fitlists[order]])
    DeltaE_l0 = np.array([delta_E_fix for _ in range(len(DeltaE))])
    lambdas = np.array([fit[f"lambdas"] for fit in fitlists[order]])
    print(f"{np.shape(DeltaE)=}")
    print(f"{np.shape(DeltaE_l0)=}")
    print(f"{np.shape(lambdas)=}")

    matrix_element = np.sqrt(DeltaE**2 - DeltaE_l0**2) / (2 * lambdas)
