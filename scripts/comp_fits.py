...
def plot_fits(data_, name, plotdir):
    ...
    plt.plot(tmin0_tmax, tmin0_redchisq, color=_colors[0], label=r"$\lambda_{\textrm{min}}=0$")
    plt.plot(tmin1_tmax, tmin1_redchisq, color=_colors[1], label=r"$\lambda_{\textrm{min}}=1$")
    plt.plot(tmin2_tmax, tmin2_redchisq, color=_colors[2], label=r"$\lambda_{\textrm{min}}=2$")
    plt.tight_layout()
    ...
    plt.figure(figsize=(5, 4))
    plt.errorbar(tmin0_tmax, np.average(tmin0_me, axis=1), np.std(tmin0_me, axis=1), color=_colors[0], label=r"$\lambda_{\textrm{min}}=0$", capsize=4, elinewidth=1, fmt=_markers[0])
    plt.errorbar(tmin1_tmax, np.average(tmin1_me, axis=1), np.std(tmin1_me, axis=1), color=_colors[1], label=r"$\lambda_{\textrm{min}}=1$", capsize=4, elinewidth=1, fmt=_markers[1])
    plt.errorbar(tmin2_tmax, np.average(tmin2_me, axis=1), np.std(tmin2_me, axis=1), color=_colors[2], label=r"$\lambda_{\textrm{min}}=2$", capsize=4, elinewidth=1, fmt=_markers[2])
    plt.tight_layout()
    plt.legend(fontsize="small")
    plt.xticks(np.arange(2,15))
    plt.xlabel(r"$\lambda_{\textrm{max}}$")
...
def plot_fits_comb(data_, name, plotdir):
    ...
    ax1.plot(lmbmin0_lmbmax_, lmbmin0_redchisq, color=_colors[0], label=rf"$\lambda_{{\textrm{{min}}}}={lmbmin0_fits_[0]['lambdas3'][0]:.3f}$")
    ax1.plot(lmbmin1_lmbmax_, lmbmin1_redchisq, color=_colors[1], label=rf"$\lambda_{{\textrm{{min}}}}={lmbmin1_fits_[0]['lambdas3'][0]:.3f}$")
    ax1.plot(lmbmin2_lmbmax_, lmbmin2_redchisq, color=_colors[2], label=rf"$\lambda_{{\textrm{{min}}}}={lmbmin2_fits_[0]['lambdas3'][0]:.3f}$")
    ...
    ax2 = ax1.twinx()
    ax2.errorbar(lmbmin0_lmbmax_, np.average(lmbmin0_me, axis=1), np.std(lmbmin0_me, axis=1), color=_colors[0], capsize=4, elinewidth=1, fmt=_markers[0], label=rf"$\lambda_{{\textrm{{min}}}}={lmbmin0_fits_[0]['lambdas3'][0]:.3f}$")
    ax2.errorbar(lmbmin1_lmbmax_, np.average(lmbmin1_me, axis=1), np.std(lmbmin1_me, axis=1), color=_colors[1], capsize=4, elinewidth=1, fmt=_markers[1], label=rf"$\lambda_{{\textrm{{min}}}}={lmbmin1_fits_[0]['lambdas3'][0]:.3f}$")
    ax2.errorbar(lmbmin2_lmbmax_, np.average(lmbmin2_me, axis=1), np.std(lmbmin2_me, axis=1), color=_colors[2], capsize=4, elinewidth=1, fmt=_markers[2], label=rf"$\lambda_{{\textrm{{min}}}}={lmbmin2_fits_[0]['lambdas3'][0]:.3f}$")
