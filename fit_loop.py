...
def main_loop():
    ...
    # chisq_vals = np.array([elem["redchisq3"]*(len(elem["lmb_range"])-2) for elem in fit_data_list])
    # print(x_coord)
    # print(y_coord)
    # print(redchisq_vals)
    # print(len(x_coord))
    # print(len(y_coord))
    # print(len(redchisq_vals))
    ...
    # mat = plt.pcolormesh(all_data["lambdas3"][unique_x], all_data["lambdas3"][unique_y], matrix.T, cmap = 'GnBu', vmin=0, vmax=4)
    ...
    mat = plt.pcolormesh(all_data["lambdas3"][unique_x], all_data["lambdas3"][unique_y], matrix.T, cmap = 'GnBu',vmax=np.max(matrix), vmin = np.min(np.where(matrix==0,1,matrix)) ) #0.65) #, norm=colors.LogNorm(vmin=1e-1, vmax=np.max(matrix)))
