...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        config_file = "data_params/data_dir_theta2_fix.yaml"
    print(f"Reading directories from: {config_file}\n")
    with open(config_file) as f:
        config = yaml.safe_load(f)

    # Set parameters to defaults defined in another YAML file
    with open("data_params/defaults.yaml") as f:
        defaults = yaml.safe_load(f)
    for key, value in defaults.items():
        config.setdefault(key, value)

    plotdir = Path.home() / Path(config["plot_dir"])
    datadir = Path.home() / Path(config["data_dir"])
